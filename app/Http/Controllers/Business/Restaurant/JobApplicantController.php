...
class JobApplicantController extends Controller
{
    ...
    public function applicants(Request $request,JobOffer $jobOffer)
    {
        try{
            if($request->ajax()){
                ...
                return DataTables::of($data)
                       ...
                       ->addColumn('actions', function ($row) {
                           return view('content.business.restaurants.job-offers.applicants.actions', [
                               'jobApplication' => $row
                           ])->render();
                        })
            ...
            return view('content.business.restaurants.job-offers.applicants.index',compact('jobOffer'));
        }catch(\Exception $e){
    ...
    public function show(JobApplication $jobApplicant)
    {
        try{
            ...
            return view('content.business.restaurants.job-offers.applicants.index');
        }catch(\Exception $e){
            return response()->json(['error' => $e->getMessage()], 500);
        }
    }
