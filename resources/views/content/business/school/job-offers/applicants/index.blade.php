...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    ...
    var table = $('.datatables-users').DataTable({
      processing: true,
      serverSide: true,
      ajax: '{!! route('school.job-offer.applicants', ['jobOffer' => $jobOffer->id]) !!}',
      columns: [
  ...
  function viewApplicant(id) {
    $.ajax({
      url: "{{ route('school.job-applicant.show', ':id') }}".replace(':id', id),
      type: 'GET',
...
<div class="w-100 d-flex align-items-center justify-content-between mb-4">
    ...
    <a href="{{route('school.job-offer.index')}}" class="btn btn-primary btn-custom">
        {{ __('Back') }}
    </a>
</div>
<!-- Users List Table -->
<div class="card">
  ...
  <div class="offcanvas offcanvas-end" tabindex="-1" id="addJobOfferModal" aria-labelledby="addJobOfferModalLabel">
    <div class="offcanvas-header">
      <h5 id="addJobOfferModalLabel" class="offcanvas-title">Add Job Offer</h5>
    ...
    <div class="offcanvas-body mx-0 flex-grow-0">
      <form class="add-new-user pt-0" id="addJobOfferForm" action="{{ route('school.job-offer.store') }}" method="POST">
        ...
        <div class="mb-3">
          ...
          <input type="text" class="form-control" id="position" name="position" placeholder="e.g. 2nd grade English Teacher" required>
          <small class="text-danger">{{ $errors->first('position') }}</small>
        </div>

        <div class="mb-3">
          <label class="form-label" for="description">Description</label>
          <textarea class="form-control" id="description" name="description" rows="3" maxlength="400" placeholder="Enter job description (max 400 chars)"></textarea>
          <small class="text-danger">{{ $errors->first('description') }}</small>
        </div>

        <div class="mb-3">
          <label class="form-label" for="location">Location</label>
          <input type="text" class="form-control" id="location" name="location" placeholder="School Address" required>
          <small class="text-danger">{{ $errors->first('location') }}</small>
        </div>

        <div class="mb-3">
          <label class="form-label" for="salary">Salary</label>
          <input type="number" class="form-control" id="salary" name="salary" placeholder="400 czk" required>
        ...
        <div class="mb-3">
          <label class="form-label" for="contact">Contact Mobile No.</label>
          <input type="tel" class="form-control" id="contact_number" name="contact_number" placeholder="Enter mobile number"
              pattern="[0-9]+" oninput="this.value = this.value.replace(/[^0-9]/g, '')" required>
          <small class="text-danger">{{ $errors->first('contact_number') }}</small>
        </div>

        <div class="mb-3">
          <label class="form-label" for="flatpickr-date">Expiry Date</label>
          <input type="text" class="form-control" id="flatpickr-date" name="expiry_date" placeholder="Select expiry date">
          <small class="text-muted">Defaults to 60 days from today if not provided.</small>
            ...
            <option value="">Select Status</option>
              @foreach (\App\Enums\SchoolJobOffer::cases() as $status)
                  <option value="{{ $status->value }}"
                      ...
                      {{ Illuminate\Support\Str::headline($status->name) }}
                  </option>
              @endforeach
          </select>
        ...
        <button type="submit" id="submitButton" class="btn btn-primary me-3 data-submit btn-custom">Submit</button>
