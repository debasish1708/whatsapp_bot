...
@section('title', __('Members - School Management'))
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function () {
      ...
      var table = $('.datatables-users').DataTable({
        processing: true,
        serverSide: true,
        ajax: '{!! route('school.members.index') !!}',
        columns: [
        ...
        buttons: [
          {
            ...
            action: function (e, dt, node, config) {
              ...
              $('#member-id').val('');
              $('#addMemberForm').attr('action', '{{ route('school.members.store') }}');
    ...
    function handleEditMember(id) {
      let url = "{{route('school.members.edit', ':id')}}".replace(':id', id);
      $.ajax({
        url: url,
        type: 'GET',
        dataType: 'json',
        success: function (data) {
          ...
          $('#addMemberForm').attr('action', "{{route('school.members.update', ':id')}}".replace(':id', id));
    ...
    function handleViewMember(id) {
      $.ajax({
        url: "{{ route('school.members.show', ':id') }}".replace(':id', id),
        type: 'GET',
        dataType: 'json',
        success: function (data) {
...
@section('content')
  @if ($errors->any())
    <script>
      document.addEventListener('DOMContentLoaded', function () {
        ...
        if (memberId) {
          // Edit mode
          $('#addMemberForm').attr('action', "{{ route('school.members.update', ':id') }}".replace(':id', memberId));
          if ($('#addMemberForm input[name="_method"]').length === 0) {
            $('#addMemberForm').append('<input type="hidden" name="_method" value="PUT">');
          }
          $('#offcanvasAddMemberLabel').text('{{ __("Edit Member") }}');
  ...
  <div class="alert alert-info d-flex align-items-center mb-4" role="alert">
    ...
    <div>
      {{ __('Members added to School can create events and send sos alerts, announcements to students via WhatsApp.') }}
    </div>
  </div>
  ...
  <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasAddMember" aria-labelledby="offcanvasAddMemberLabel">
    ...
    <div class="offcanvas-body mx-0 flex-grow-0 p-6 h-100">
      <form id="addMemberForm" class="pt-0" action="{{ route('school.members.store') }}" method="POST">
        @csrf
