...
@section('page-script')
  ...
  <script>
    ...
    function fetchChatHistory(minDate = null, maxDate = null, callback = null) {
      $.ajax({
      url: "{{route('chat-history.show', $user->id)}}",
      method: 'GET',
      data: {
        minDate: minDate,
        maxDate: maxDate
      ...
      success: function (result) {
        ...
        $('#chat-history').html('No chats found');
        if (callback) callback();
        return;
        }
        result.data.forEach((chat) => {
        let data = '';
        if (chat.request != null && chat.request != '') {
          ...
          data += '<li class="chat-message">' +
          '<div class="d-flex overflow-hidden">' +
          '<div class="user-avatar flex-shrink-0 me-4">' +
          '<div class="avatar avatar-sm">' +
          '<img src="{{asset('assets/img/user-placeholder.png')}}" alt="Avatar" class="rounded-circle">' +
          '</div>' +
          '</div>' +
        ...
        if (chat.response != null && chat.response != '') {
          const date = new Date(chat.created_at);
          const formatted = date.getFullYear() + '-' +
          String(date.getMonth() + 1).padStart(2, '0') + '-' +
          String(date.getDate()).padStart(2, '0') + ' ' +
          ...
          data += '<li class="chat-message chat-message-right w-100">' +
          ...
          '<div class="avatar avatar-sm">' +
          '<img src="{{asset('assets/img/favicon/CITIO_LOGO.png')}}" alt="Avatar" class="rounded-circle">' +
          '</div>' +
          '</div>' +
          '</div>' +
          '</li>';
    ...
    // Date filter handler
    $('#flatpickr-range').on('input', function () {
      var dates = $('#flatpickr-range').val().split(' to ');
      var minDate, maxDate;
      if (dates.length == 1) {
      minDate = dates[0];
      maxDate = dates[0];
      ...
      fetchChatHistory(minDate, maxDate);
    });

    // Refresh button handler
    const refreshBtn = document.getElementById('refresh-chat-btn');
    refreshBtn.addEventListener('click', function () {
      var dateVal = $('#flatpickr-range').val();
      var minDate = null, maxDate = null;
      if (dateVal) {
      var dates = dateVal.split(' to ');
      if (dates.length == 1) {
        minDate = dates[0];
        maxDate = dates[0];
      } else {
      ...
      fetchChatHistory(minDate, maxDate, function () {
      // Restore button after AJAX
      refreshBtn.disabled = false;
      refreshBtn.innerHTML = originalIcon;
      });
    });

    // Optionally, fetch all chat history on page load
    // fetchChatHistory();
    });
  </script>
@endsection
  ...
  <div class="app-chat card overflow-hidden">
    ...
    <div class="col app-chat-history">
      ...
      <div class="chat-history-header border-bottom">
        ...
        <div class="d-flex overflow-hidden align-items-center">
          <i class="ti ti-menu-2 ti-lg cursor-pointer d-lg-none d-block me-4" data-bs-toggle="sidebar" data-overlay
          ...
          <small class="user-status text-body">{{$user->mobile_number}}</small>
          </div>
        </div>
        <div class="d-flex align-items-center gap-2">
          <input type="text" class="form-control" placeholder="{{ __('Filter chats by date') }}" id="flatpickr-range" />
          <button type="button" class="btn btn-icon btn-primary rounded-circle ms-2" id="refresh-chat-btn"
          data-bs-toggle="tooltip" data-bs-placement="top" title="Refresh Chat">
          <i class="ti ti-refresh ti-md"></i>
          </button>
        </div>
        </div>
      </div>
      <div class="chat-history-body" style="transition: all 0.3s ease;">
        <ul class="list-unstyled chat-history" id="chat-history">
        @if($chat_history->count() == 0)
      {{ __('No chats found.') }}
      @endif
        @foreach($chat_history as $chat)
        @if($chat->request && $chat->request != '')
        ...
        <div class="text-muted mt-1">
        <small>{{\Carbon\Carbon::parse($chat->created_at)->format("Y-m-d H:i")}}</small>
        </div>
        </div>
        </div>
        </li>
        ...
        <li class="chat-message chat-message-right w-100">
        <div class="d-flex overflow-hidden w-75">
        <div class="chat-message-wrapper flex-grow-1">
        <div class="chat-message-text">
        <p class="mb-0">{{$chat->response}}</p>
      ...
      @endforeach
        {{-- <li class="chat-message chat-message-right">
          ...
          <div class="chat-message-wrapper flex-grow-1">
            <div class="chat-message-text">
            <p class="mb-0">{{ config('variables.templateName') ? config('variables.templateName') :
              'TemplateName' }} has all the components you'll ever need in a app.</p>
            </div>
            <div class="text-end text-muted mt-1">
            <i class='ti ti-checks ti-16px text-success me-1'></i>
        ...
        <li class="chat-message">
          ...
          <div class="chat-message-wrapper flex-grow-1">
            <div class="chat-message-text">
            <p class="mb-0">Do you have design files for {{ config('variables.templateName') ?
              config('variables.templateName') : 'TemplateName' }}?</p>
            </div>
            <div class="text-muted mt-1">
    ...
    {{-- <div class="col app-chat-sidebar-right app-sidebar overflow-hidden" id="app-chat-sidebar-right">
      ...
      <div class="d-flex mt-6">
        ...
        data-target="#app-chat-sidebar-right">Delete Contact<i class='ti ti-trash ti-16px ms-2'></i></button>
      </div>
      </div>
    </div> --}}
    <!-- /Sidebar Right -->
