...
@section('title', 'School Profile - Profile')
...
<div class="w-100 d-flex align-items-center justify-content-between mb-4">
    <h4 class="mb-0">Schools Details</h4>
    <div class="d-flex gap-2">
        @if($school->is_profile_completed && $school->user->status == 'pending')
            <!-- Full Approve Button -->
            <a href="javascript:void(0)" onclick="handleStatusButtons('{{route('schools.approve', $school->id)}}', 'approve')"
               class="btn btn-success">
                Approve
            </a>

            <!-- Full Reject Button -->
            <a href="javascript:void(0)" onclick="handleStatusButtons('{{route('schools.reject', $school->id)}}', 'reject')"
        ...
        <!-- Back Button -->
        <a href="{{ route('schools.index') }}" class="btn btn-back btn-custom ms-3">
            Back
        </a>
    </div>
</div>
...
<div class="row">
    ...
    <div class="card mb-6">
      <div class="card-body">
        ...
        <ul class="list-unstyled my-3 py-1">
          <li class="d-flex align-items-center mb-4"><i class="ti ti-user ti-lg"></i><span class="fw-medium mx-2">School Name:</span> <span>{{$school->user->name ?? ''}}</span></li>
          <li class="d-flex align-items-center mb-4"><i class="ti ti-building ti-lg"></i><span class="fw-medium mx-2">City:</span> <span>{{$school->city ?? ''}}</span></li>
        ...
        <ul class="list-unstyled my-3 py-1">
          <li class="d-flex align-items-center mb-4"><i class="ti ti-phone-call ti-lg"></i><span class="fw-medium mx-2">Contact:</span> <span>{{$school->mobile_number ?? ''}}</span></li>
          <li class="d-flex align-items-center mb-4" style="display: inline-block !important; flex-shrink:1;"><i class="ti ti-mail ti-lg"></i><span class="fw-medium mx-2">Email:</span> <span>{{$school->user->email ?? ''}}</span></li>
  ...
  <div class="col-xl-8 col-lg-7 col-md-7">
    <div class="card mb-6">
      <div class="card-body">
        {{-- <small class="card-text text-uppercase text-muted small">Details</small> --}}
        <small class="card-text text-uppercase text-muted small">School logo</small>
        ...
        <ul class="list-unstyled mb-0 mt-3 pt-1">
          {{-- @if($restaurant->sustainabilities)
            ...
            @endforeach
          @endif --}}
          <img src="{{$school->logo ?? asset('assets/img/placeholder.jpg') }}" alt="" width="130" height="130"/>
        </ul>
      </div>
    </div>
    <!--/ About User -->
    ...
    <div class="card mb-6">
      <div class="card-body">
        <small class="card-text text-uppercase text-muted small">Services</small>
        <div class="w-75 d-flex flex-wrap gap-2 mt-3">
          @if($school->services)
            @foreach(collect(json_decode($school->services))->toArray() as $service)
              {{-- <li class="d-flex flex-wrap mb-4"><span class="fw-medium me-2">{{$service}}</span></li> --}}
              <span class="badge bg-primary-subtle text-primary rounded-pill px-3 py-2">
                <i class="ti ti-check me-1"></i> {{$service}}
              </span>
            @endforeach
          @else
